...
export function OddsCompare({ odds, region }: Props) {
  const visible = odds.filter((o) => {
    ...
    return book ? book.regions.includes(region as never) : true;
  });
  ...
  return (
    <ul className="grid gap-2">
      {visible.map((o) => {
        ...
        const name = book?.name ?? o.bookCode.replace(/_/g, ' ');
        return (
          <li key={`${o.bookCode}-${o.market}`} className={`flex items-center justify-between rounded-xl bg-ink-2 px-4 py-3 ring-1 ${isBest ? 'ring-edge-cyan/40' : 'ring-white/10'}`}>
            <div>
